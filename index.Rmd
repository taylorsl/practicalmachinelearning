...
I used the Random Forest model for training and achieved 99.82% accuracy on the validation set. The training set was 60% of the original training data, with 40% used for the cross-validation. I determined the set of features by eliminating those that were time stamps and obviously low variance immediately, checking for other near zero variance features and then eliminating variables with high cross-correlation. This brought us to 32 features used in the training set. 
